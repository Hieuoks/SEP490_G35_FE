...
const navigate = useNavigate();
  ...
  // Hàm tạo link Zalo với số điện thoại
  const getZaloLink = (phone) => {
  ...
  <div className="card-body">
    <h5
      className="fs-18 mb-3 text-primary"
      style={{ cursor: "pointer" }}
      onClick={() => navigate(`/tour-operator/detail/${tour?.tourOperatorId}`)}
    ...
    <div className="bg-light-500 br-10 mb-3 d-flex align-items-center p-3">
      <div className="avatar avatar-lg flex-shrink-0 me-2">
        <img
          ...
          alt={tour?.companyName || "Logo công ty"}
          className="rounded-circle"
          style={{ width: 64, height: 64, objectFit: "cover" }}
          onError={e => {
            e.target.onerror = null;
            e.target.src = "/assets/img/users/user-05.jpg";
          }}
        />
      </div>
      <div className="ms-2 overflow-hidden">
        ...
        <p className="fs-14 mb-0">
          Tham gia từ:{" "}
          {tour?.createdAt ? formattedDate(tour.createdAt) : "Không rõ"}
        </p>
      </div>
    </div>
    ...
    <div className="row g-2">
      <div className="col-12">
        <a
          href={getZaloLink(tour?.companyHotline)}
          target="_blank"
        ...
        >
          ...
          {tour?.companyHotline
            ? `Liên hệ Zalo (${tour.companyHotline})`
            : "Liên hệ Zalo"}
        </a>
      </div>
    </div>
